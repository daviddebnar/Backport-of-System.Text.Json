...
namespace System.Text.Json.Serialization
{
    ...
    /// </summary>
    internal abstract class JsonDictionaryConverterS<TDictionary> : JsonResumableConverterS<TDictionary>
        where TDictionary : struct
    {
    ...
    /// Base class for dictionary converters such as IDictionary, Hashtable, Dictionary{,} IDictionary{,} and SortedList.
    /// </summary>
    internal abstract class JsonDictionaryConverterS<TDictionary, TKey, TValue> : JsonDictionaryConverterS<TDictionary>
        where TDictionary : struct
        where TKey : struct
        where TValue : struct
    {
        ...
        protected JsonConverterS<TKey>? _keyConverter;
        protected JsonConverterS<TValue>? _valueConverter;

        protected static JsonConverterS<T> GetConverter<T>(JsonTypeInfo typeInfo) where T : struct
        {
            JsonConverterS<T> converter = (JsonConverterS<T>)typeInfo.PropertyInfoForTypeInfo.ConverterBase;
            ...
            if (state.UseFastPath)
            {
                ...
                if (_valueConverter.CanUseDirectReadOrWrite && state.Current.NumberHandling == null)
                {
                    ...
                    while (true)
                    {
                        ...
                        Add(key, element!.Value, options, ref state);
                    }
                }
                else
                {
