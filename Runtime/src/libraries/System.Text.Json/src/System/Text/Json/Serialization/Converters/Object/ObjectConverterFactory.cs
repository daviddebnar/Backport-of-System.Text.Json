...
namespace System.Text.Json.Serialization.Converters
{
    ...
    internal sealed class ObjectConverterFactory : JsonConverterFactory
    {
        ...
        public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            {
                converterType = typeof(ObjectDefaultConverterC<>).MakeGenericType(typeToConvert);
            }
            else
            {
                ...
                if (parameterCount <= JsonConstants.UnboxedParameterCountThreshold)
                {
                    ...
                    converterType = typeof(SmallObjectWithParameterizedConstructorConverterC<,,,,>).MakeGenericType(typeArguments);
                }
                else
                {
                    converterType = typeof(LargeObjectWithParameterizedConstructorConverterWithReflectionC<>).MakeGenericType(typeToConvert);
                }
            }
        ...
        private JsonConverter CreateKeyValuePairConverter(Type type, JsonSerializerOptions options)
        {
            ...
            JsonConverter converter = (JsonConverter)Activator.CreateInstance(
                typeof(KeyValuePairConverterC<,>).MakeGenericType(new Type[] { keyType, valueType }),
                BindingFlags.Instance | BindingFlags.Public,
                binder: null,
                args: null,
                culture: null)!;
