...
namespace System.Text.Json.Serialization.Converters
{
    ...
    internal sealed class JsonMetadataServicesConverterC<T> : JsonResumableConverterC<T> where T : class
    {
        private readonly Func<JsonConverterC<T>> _converterCreator;

        private readonly ConverterStrategy _converterStrategy;

        private JsonConverterC<T>? _converter;

        // A backing converter for when fast-path logic cannot be used.
        internal JsonConverterC<T> Converter
        {
            get
            {
        ...
        public JsonMetadataServicesConverterC(Func<JsonConverterC<T>> converterCreator, ConverterStrategy converterStrategy)
        {
        ...
        internal override bool OnTryWrite(Utf8JsonWriter writer, T value, JsonSerializerOptions options, ref WriteStack state)
        {
            ...
            if (!state.SupportContinuation &&
                jsonTypeInfo is JsonTypeInfoC<T> info &&
