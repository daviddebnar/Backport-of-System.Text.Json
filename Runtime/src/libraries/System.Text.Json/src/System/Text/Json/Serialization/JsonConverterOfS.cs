...
namespace System.Text.Json.Serialization
{
    ...
    public abstract partial class JsonConverterS<T> : JsonConverter where T : struct
    {
        /// <summary>
        /// When overidden, constructs a new <see cref="JsonConverterS{T}"/> instance.
        /// </summary>
        protected internal JsonConverterS()
        {
            ...
            CanBeNull = false; // default(T) is null;
        ...
        {
            return new JsonPropertyInfoS<T>();
        }
        ...
        {
            value = Read(ref reader, typeToConvert, options)!.Value;
            return true;
        }
        ...
        internal bool TryWriteDataExtensionProperty(Utf8JsonWriter writer, T value, JsonSerializerOptions options, ref WriteStack state)
        {
            ...
            JsonDictionaryConverterS<T, T, T>? dictionaryConverter = this as JsonDictionaryConverterS<T, T, T>
                                                                ?? (this as JsonMetadataServicesConverterS<T>)?.Converter as JsonDictionaryConverterS<T, T, T>;
        ...
        public virtual T ReadAsPropertyName(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (!IsInternalConverter && options.TryGetDefaultSimpleConverter(TypeToConvert, out JsonConverter? defaultConverter))
            {
                ...
                Debug.Assert(defaultConverter.IsInternalConverter && defaultConverter is JsonConverterS<T>);
                return ((JsonConverterS<T>)defaultConverter).ReadAsPropertyNameCore(ref reader, TypeToConvert, options);
            }
        ...
        public virtual void WriteAsPropertyName(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
        {
            if (!IsInternalConverter && options.TryGetDefaultSimpleConverter(TypeToConvert, out JsonConverter? defaultConverter))
            {
                // .NET 5 backward compatibility: hardcode the default converter for primitive key serialization.
                Debug.Assert(defaultConverter.IsInternalConverter && defaultConverter is JsonConverterS<T>);
                ((JsonConverterS<T>)defaultConverter).WriteAsPropertyNameCore(writer, value, options, isWritingExtensionDataProperty: false);
                return;
            }
