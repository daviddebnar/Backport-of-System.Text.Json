...
namespace System.Text.Json.Serialization.Converters
{
    ...
    /// reference to an actual <see cref="JsonConverterC{T}"/> for the type
    ...
    internal sealed class JsonMetadataServicesConverterS<T> : JsonResumableConverterS<T> where T : struct
    {
        private readonly Func<JsonConverterS<T>> _converterCreator;

        private readonly ConverterStrategy _converterStrategy;

        private JsonConverterS<T>? _converter;

        // A backing converter for when fast-path logic cannot be used.
        internal JsonConverterS<T> Converter
        {
            get
            {
        ...
        public JsonMetadataServicesConverterS(Func<JsonConverterS<T>> converterCreator, ConverterStrategy converterStrategy)
        {
        ...
        internal override bool OnTryWrite(Utf8JsonWriter writer, T value, JsonSerializerOptions options, ref WriteStack state)
        {
            ...
            if (!state.SupportContinuation &&
                jsonTypeInfo is JsonTypeInfoS<T> info &&
