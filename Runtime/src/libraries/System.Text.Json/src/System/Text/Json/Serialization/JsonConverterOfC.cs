...
namespace System.Text.Json.Serialization
{
    ...
    public abstract partial class JsonConverterC<T> : JsonConverter where T : class
    {
        /// <summary>
        /// When overidden, constructs a new <see cref="JsonConverterC{T}"/> instance.
        /// </summary>
        protected internal JsonConverterC()
        {
        ...
        {
            return new JsonPropertyInfoC<T>();
        }
        ...
        internal bool TryWriteDataExtensionProperty(Utf8JsonWriter writer, T value, JsonSerializerOptions options, ref WriteStack state)
        {
            ...
            JsonDictionaryConverterC<T, T, T>? dictionaryConverter = this as JsonDictionaryConverterC<T, T, T>
                                                                ?? (this as JsonMetadataServicesConverterC<T>)?.Converter as JsonDictionaryConverterC<T, T, T>;
        ...
        public virtual T ReadAsPropertyName(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (!IsInternalConverter && options.TryGetDefaultSimpleConverter(TypeToConvert, out JsonConverter? defaultConverter))
            {
                ...
                Debug.Assert(defaultConverter.IsInternalConverter && defaultConverter is JsonConverterC<T>);
                return ((JsonConverterC<T>)defaultConverter).ReadAsPropertyNameCore(ref reader, TypeToConvert, options);
            }
        ...
        public virtual void WriteAsPropertyName(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
        {
            if (!IsInternalConverter && options.TryGetDefaultSimpleConverter(TypeToConvert, out JsonConverter? defaultConverter))
            {
                // .NET 5 backward compatibility: hardcode the default converter for primitive key serialization.
                Debug.Assert(defaultConverter.IsInternalConverter && defaultConverter is JsonConverterC<T>);
                ((JsonConverterC<T>)defaultConverter).WriteAsPropertyNameCore(writer, value, options, isWritingExtensionDataProperty: false);
                return;
            }
