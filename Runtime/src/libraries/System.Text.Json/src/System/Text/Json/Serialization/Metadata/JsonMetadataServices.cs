...
namespace System.Text.Json.Serialization.Metadata
{
    ...
    public static partial class JsonMetadataServices
    {
        ...
        public static JsonPropertyInfo CreatePropertyInfoC<T>(JsonSerializerOptions options, JsonPropertyInfoValuesC<T> propertyInfo)
            where T : class
        {
            if (options == null)
            {
            ...
            if (converter == null)
            {
                converter = propertyTypeInfo.PropertyInfoForTypeInfo.ConverterBase as JsonConverterC<T>;
                if (converter == null)
                {
            ...
            JsonPropertyInfoC<T> jsonPropertyInfo = new JsonPropertyInfoC<T>();
        ...
        /// <param name="propertyInfo">Provides serialization metadata about the property or field.</param>
        /// <returns>A <see cref="JsonPropertyInfo"/> instance intialized with the provided metadata.</returns>
        public static JsonPropertyInfo CreatePropertyInfoS<T>(JsonSerializerOptions options, JsonPropertyInfoValuesS<T> propertyInfo)
            where T : struct
        {
            if (options == null)
            {
            ...
            if (converter == null)
            {
                converter = propertyTypeInfo.PropertyInfoForTypeInfo.ConverterBase as JsonConverterS<T>;
                if (converter == null)
                {
                    throw new InvalidOperationException(SR.Format(SR.ConverterForPropertyMustBeValid, declaringType, propertyName, typeof(T)));
                }
            ...
            JsonPropertyInfoS<T> jsonPropertyInfo = new JsonPropertyInfoS<T>();
            jsonPropertyInfo.InitializeForSourceGen(options, propertyInfo);
            return jsonPropertyInfo;
        }
        ...
        public static JsonTypeInfoC<T> CreateObjectInfoC<T>(JsonSerializerOptions options, JsonObjectInfoValuesC<T> objectInfo) 
            where T : class
            => new JsonTypeInfoInternalC<T>(
        ...
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="options"/> or <paramref name="objectInfo"/> is null.</exception>
        /// <returns>A <see cref="JsonTypeInfo{T}"/> instance representing the class or struct.</returns>
        public static JsonTypeInfoS<T> CreateObjectInfoS<T>(JsonSerializerOptions options, JsonObjectInfoValuesS<T> objectInfo) 
            where T : struct
            => new JsonTypeInfoInternalS<T>(
                options ?? throw new ArgumentNullException(nameof(options)),
                objectInfo ?? throw new ArgumentNullException(nameof(objectInfo)));
        ...
        public static JsonTypeInfoC<T> CreateValueInfoC<T>(JsonSerializerOptions options, JsonConverter converter)
            where T : class
        {
            JsonTypeInfoC<T> info = new JsonTypeInfoInternalC<T>(options);
        ...
        /// <typeparam name="T">The generic type definition.</typeparam>
        /// <returns>A <see cref="JsonTypeInfo{T}"/> instance representing the type.</returns>
        public static JsonTypeInfoS<T> CreateValueInfoS<T>(JsonSerializerOptions options, JsonConverter converter)
            where T : struct
        {
            JsonTypeInfoS<T> info = new JsonTypeInfoInternalS<T>(options);
            info.PropertyInfoForTypeInfo = CreateJsonPropertyInfoForClassInfo(typeof(T), info, converter, options);
            return info;
        }
